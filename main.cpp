...
bool checkColissionBox(const char* object1, const char* object2);
bool checkColissionPoint(glm::vec4 point, const char* box);
...
float prev_x1 = 12.35f;
float prev_y1 = 2.50f;
float prev_z1 = -85.13f;
    ...
    while ((!glfwWindowShouldClose(window))||(glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS))
    {
        ...
        for(int j=0; j<tree_types; j++){
            for(i=current_i; i<(current_i)+amount; i++){
                ...
                collision = checkColissionPoint(camera_position_c, tree_names[j]);
                if(collision){
                    camera_position_c = glm::vec4(prev_x1, prev_y1, prev_z1, 1.0f);
                    x1 = prev_x1;
                    y1 = prev_y1;
                    z1 = prev_z1;
                }
...
bool checkColissionBox(const char* object1, const char* object2){
    ...
    glm::vec3 bbox_min1 = g_VirtualScene[object1].bbox_min;
    glm::vec3 bbox_max1 = g_VirtualScene[object1].bbox_max;

    glm::vec3 bbox_min2 = g_VirtualScene[object2].bbox_min;
    glm::vec3 bbox_max2 = g_VirtualScene[object2].bbox_max;

    return (
    bbox_min1.x <= bbox_max2.x &&
    bbox_max1.x >= bbox_min2.x &&
    bbox_min1.y <= bbox_max2.y &&
    bbox_max1.y >= bbox_min2.y &&
    bbox_min1.z <= bbox_max2.z &&
    bbox_max1.z >= bbox_min2.z
  );
}
/*bool checkColissionSphere(const char* sphere, const char* box){

    glm::vec3 bbox_min = g_VirtualScene[box].bbox_min;
    glm::vec3 bbox_max = g_VirtualScene[box].bbox_max;

    const x = max(bbox_min.x, min(sphere.x, bbox_max.x))
    const y = max(bbox_min.y, min(sphere.y, bbox_max.y))
    const z = max(bbox_min.z, min(sphere.z, bbox_max.z))

    const distance = sqrt((x - sphere.x) * (x - sphere.x) +
